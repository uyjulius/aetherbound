...
extends RefCounted
##
## The first volume of scripted scenes: the critical path's five bosses, the ending,
## the cataclysm that turns the world over, and eleven recruitments.
##
## A translation of `src/data/events.js`, kept line for line with it. Each scene is a
## coroutine that awaits an `EventContext` — `yield* say(game, 'Vesna', [...])` there
## is `await ctx.say("Vesna", [...])` here — so the two can be read side by side and
## `tools/events-parity.mjs` compares what they actually do, call for call, under five
## branch policies.
##
## Dispatch is a `match` rather than a table of `Callable`s because a static method
## reference resolved through a name is exactly the kind of thing that fails silently
## when a scene is renamed, and a missing branch here is a loud error instead.
...
const IDS := [
	...
	"recruit_osric", "recruit_maret", "cataclysm", "recruit_tam", "recruit_ilsabet",
	"recruit_kestrel", "recruit_oda", "recruit_rusk", "recruit_mask", "barrow_hint",
]
...
static func fenbarrow_boss(ctx: EventContext) -> void:
	...
	var result := await ctx.battle({"enemies": ["bogfather"]},
	...
	await ctx.grant_chest({"kind": "esper", "id": "hollowking",
		"label": "a shard of magicite"}, ctx.field)
	await ctx.say("Vesna", [
...
static func recruit_aurelian(ctx: EventContext) -> void:
	...
	if not ctx.has_flag("bogfather_slain"):
		await ctx.say("Aurelian Marchetti", [
	...
	# The reference clears the weapon slot on the new member here; that is state on
	# the member rather than anything a scene's transcript can see.
	ctx.recruit("aurelian")
...
static func recruit_bastian(ctx: EventContext) -> void:
	if ctx.in_roster("bastian"):
		await ctx.say("Bastian", [
			"Say the word and I will put someone through a wall for you. Professionally.",
		])
		return
	if not ctx.has_flag("aurelian_joined"):
		await ctx.say("Bastian Marchetti", [
...
static func ferran_warden(ctx: EventContext) -> void:
	...
	var result := await ctx.battle({"enemies": ["ferranwarden"]},
...
## Ashenhall's reliquary. The Eighth Lantern has kept vigil for a thousand years and
## does not intend to be relieved.
static func ashenhall_reliquary(ctx: EventContext) -> void:
	...
	var result := await ctx.battle({"enemies": ["eighthlantern"]},
	...
	await ctx.grant_chest({"kind": "esper", "id": "ninthlantern",
		"label": "the Ninth Lantern"}, ctx.field)
	await ctx.say("Vesna", [
...
## The Standing Oak, on the world map. Optional, and the reward is a healing esper —
## the kind of thing a player who wanders should find.
static func standing_oak(ctx: EventContext) -> void:
	if ctx.has_flag("oak_slain"):
		...
		return
	var choice := await ctx.ask(
		"The oak turns to face you. It is far too old to be a tree, and it knows your name.",
		["Stand your ground", "Back away slowly"], {"cancelable": true})
	ctx.close_dialogue()
	if choice != 0:
		return

	var result := await ctx.battle({"enemies": ["greenmother_guardian"]},
	...
	await ctx.grant_chest({"kind": "esper", "id": "greenmother",
		"label": "a shard of magicite"}, ctx.field)
	ctx.complete_quest("oak")


## The Toll Baron ambushes the road once the player has been through Solmere. A pure
## sidequest: no bearing on the plot and excellent loot.
static func toll_baron(ctx: EventContext) -> void:
	...
	var result := await ctx.battle({"enemies": ["tollbaron", "brigand", "brigand"]},
	...
	ctx.add_gold(2500)
	await ctx.say(null, [
		"His strongbox holds 2500 gil and a great deal of other people's correspondence.",
	])
...
static func cinderspine_wyrm(ctx: EventContext) -> void:
	...
	var result := await ctx.battle({"enemies": ["cinderwyrm"]},
...
static func ninthwell_warden(ctx: EventContext) -> void:
	...
	var result := await ctx.battle({"enemies": ["enginewarden"]},
...
static func ninthwell_finale(ctx: EventContext) -> void:
	...
	var result := await ctx.battle({"enemies": ["vhaineshadow"]},
		{"boss": true, "terrain": "cave", "scenery": "cave", "canFlee": false})
	if result != "victory":
		return
...
## Idris, at the burnt shrine. The last retainer of a house that no longer exists.
...
## Osric, sitting in his own shipwreck. Joins for the least noble reason of anyone in
## the party, which is why he is the easiest to trust.
...
## Maret defects at the outpost, once the Warden has shown her what the Imperium has
...
static func recruit_maret(ctx: EventContext) -> void:
	...
	if not ctx.has_flag("warden_slain"):
		await ctx.say("General Maret Sunder", [
...
## The cataclysm — the hinge of the game. The same continent, the same road, the same
## towns, all read differently afterwards, and deliberately not a new set of maps.
static func cataclysm(ctx: EventContext) -> void:
	...
	await ctx.over(2.4, func(t: float, _dt: float): ctx.shake(0.9 * (1.0 - t * 0.4), 1.6))
	await ctx.say(null, [
	...
	# The second sweep only fades the flash, which is not a decision a transcript can
	# see — but it is stepped anyway, because the reference spends the time.
	await ctx.over(1.4, func(_t: float, _dt: float): pass)
	...
	# The airship is handed over here rather than earned in a side scene: the ruined
	# world is where the map stops being a road with places on it and becomes a place
	# to search, and the player needs the means in the same breath as the reason.
	ctx.set_flag("airship")
	await ctx.say("Corvin", [
...
static func recruit_ilsabet(ctx: EventContext) -> void:
	if ctx.in_roster("ilsabet"):
		await ctx.say("Ilsabet", [
			"Hold still. Not for a portrait. I want your proportions for something worse.",
		])
		return
	await ctx.cinematic(true)
	await ctx.say("Ilsabet Rook", [
...
## The Mask. Turns up at the Ninth Well after the world breaks, and will not explain
## itself. The last recruit, and the only optional one.
static func recruit_mask(ctx: EventContext) -> void:
	...
	if ctx.world_state != "ruin":
		await ctx.say(null, [
			"A figure stands at the shaft head with its back to you. When you look again, there is nobody there.",
		])
		return
	await ctx.cinematic(true)
	await ctx.say(null, [
	...
	var choice := await ctx.ask("It waits.", ["Bow", "Speak to it", "Walk past"],
		{"cancelable": true})
	if choice == 2 or choice < 0:
		ctx.close_dialogue()
		await ctx.cinematic(false)
...
## Reading the roadside plaque the first time nudges toward the fen.
